...
我们修改name_node.py中的`new_fat_item`函数，为了负载均衡，我们在分发数据块时，每次轮流给每个节点分发数据块：
...
此外，由于数据集较大，进行分发所耗费的时间过长，我们修改了common.py中的一些配置，将`dfs_blk_size`与`BUF_SIZE`进行了增大，并且将`dfs_replication`调整为1：
